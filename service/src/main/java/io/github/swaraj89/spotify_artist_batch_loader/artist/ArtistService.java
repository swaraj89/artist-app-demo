...
public class ArtistService {
    ...
    public Page<Artist> getFilteredArtists(String country, String artistName, Pageable pageable) {
        Specification<Artist> spec = Specification.<Artist>unrestricted();

        if (country != null) {
            spec = spec.and(ArtistQuerySpecification.hasCountry(country));
        }
        if (artistName != null) {
            spec = spec.and(ArtistQuerySpecification.hasArtistName(artistName));
        }
        return artistRepository.findAll(spec, pageable);
    }

    public List<String> getDistinctCountries() {
        return artistRepository.findDistinctCountries();
    }
}
